...
export class Health extends Component {
  ...
  createNotification = (type) => {
    return () => {
      ...
      switch (type) {
        ...
        case "healthcheck":
          NotificationManager.warning(
            "Medical appointment is scheduled at 7 PM",
            "Free Time",
            2000
          );
          break;
  ...
  getPrediction = async () => {
    ...
    await axios
      .post("http://localhost:5000/getHealthPredictions", data)
      .then((response) => {
    ...
    setTimeout(this.createNotification("water"), 5000);
    setTimeout(this.createNotification("eat"), 10000);
    setTimeout(this.createNotification("exercise"), 15000);
    setTimeout(this.createNotification("sugarcheck"), 20000);
    setTimeout(this.createNotification("healthcheck"), 25000);
    setTimeout(this.createNotification("partyloc"), 30000);
    setTimeout(this.createNotification("sweetshoploc"), 35000);
  };
  ...
  troublesListf() {
    ...
    return this.state.predictions.map(function (data) {
    ...
    return (
      <div>
        ...
        <div>
          ...
          <h4>
            <center>{this.troublesListf()}</center>
          </h4>
        </div>
